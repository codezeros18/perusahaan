...
export default function ElectricalDetail() {
  const { electricalId } = useParams<{ electricalId: string }>();
  const data = electricalId ? electricalData[electricalId] : null;
  const [zoomSrc, setZoomSrc] = useState<string | null>(null);

  if (!data) {
    return <p className="p-6">electrical tidak ditemukan.</p>;
  }
